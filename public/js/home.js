...
addForm.addEventListener("submit", async (e) => {
    ...
    const response = await fetch(
        `https://financial-tracker-wine.vercel.app/add`,
        {
            method: "post",
            body: urlParams,
        }
...
document.addEventListener("click", async (e) => {
    if (e.target.closest("form")?.classList.contains("deletarForm")) {
        ...
        const response = await fetch(
            `https://financial-tracker-wine.vercel.app/deletar`,
            {
                method: "post",
                body: urlParams,
            }
    ...
    if (e.target.closest("button")?.classList.contains("editarBtn")) {
        ...
        if (tr.classList.contains("editando")) {
            ...
            const response = await fetch(
                `https://financial-tracker-wine.vercel.app/editar?tipoEd=${tipoEd}&categoriaEd=${categoriaEd}&valorEd=${valorEd}&dataEd=${dataEd}&idAtual=${idAtual}`
            );
            const data = await response.json();
